    ...
    do {
        ...
        switch(choice) {
            case 1:
                ...
                cout << decimalNum << " base 10 = " << binary << " base 2\n";
                break;
            case 2:
                // FIXME4 #FIXED
                cout << "Enter a binary number: ";
                cin >> binary;
                cout << binary << " base 2 = " << binToDec(binary) << " base 10\n";
                break;
            case 3:
                cout << "Enter a positive decimal number: ";
                cin >> decimalNum;
...
string decToBin(llu num) {
    ...
    while (quotient != 0) {
        ...
        decimal.push_back(remainder);
    }

    while(!decimal.empty()) {
        answer += to_string(decimal.back());
        decimal.pop_back();
    }
...
llu binToDec(string binaryNumber)
{
    ...
    vector<int> binary;
        for (unsigned int i = 0; i < binaryNumber.length(); i++) {
            binary.push_back(binaryNumber[i]);
        }
    while(binary.size() != 0) {
        sum += (binary.back() - int('0')) * pow(2, i);
        i += 1;
        binary.pop_back();
    }
        return sum;
